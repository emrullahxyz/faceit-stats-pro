...
if (process.env.DATABASE_URL && !process.env.DATABASE_URL.startsWith("file:")) {
    // Surface the discard instead of silently using the default path — a
    // misconfigured deployment would otherwise write to the wrong database.
    console.warn(
        "[prisma] DATABASE_URL is not a file: URL and was ignored; using file:./prisma/dev.db"
    );
}
